...
const StockUpdateTable = () => {
        ...
        try {
            ...
            if (
                ...
                if (selectedProductType === "additive") {
                    ...
                    const testItems = allProducts.filter(
                        (p) =>
                    ...
                    if (testItems.length > 0) {
                        console.warn(
                            "⚠️ Test items found in Additive collection:",
                            testItems.map((p) => ({
                                name: p.name,
                                code: p.code,
                                _id: p._id,
                ...
                const expectedItemType =
                    productTypeMap[selectedProductType] || selectedProductType;
                ...
                console.log("Filtering products:", {
                    ...
                    allProductsItemTypes: allProducts.map((p) => ({
                        name: p.name,
                        item_type: p.item_type,
                ...
                const combinedList = allProducts
                    .filter((product) => {
                        ...
                        if (selectedProductType === "tinters") {
                            ...
                            if (!matches) {
                                console.log(
                                    "Tinters filter - product rejected:",
                                    product.name,
                                    {
                                        itemType,
                        ...
                        if (selectedProductType === "additive") {
                            ...
                            if (!matches) {
                                console.log(
                                    "Additive filter - product rejected:",
                                    product.name,
                                    { itemType },
                                );
                            }
                        ...
                        if (selectedProductType === "binder") {
                            ...
                            if (!matches) {
                                console.log(
                                    "Binder filter - product rejected:",
                                    product.name,
                                    { itemType },
                                );
                            }
                        ...
                        if (selectedProductType === "auxiliary") {
                            ...
                            if (!matches) {
                                console.log(
                                    "Auxiliary filter - product rejected:",
                                    product.name,
                                    { itemType },
                                );
                            }
                        ...
                        if (selectedProductType === "accessory") {
                            ...
                            if (!matches) {
                                console.log(
                                    "Accessory filter - product rejected:",
                                    product.name,
                                    { itemType },
                                );
                            }
                        ...
                        if (selectedProductType === "third_party") {
                            ...
                            if (!matches) {
                                console.log(
                                    "Third_party filter - product rejected:",
                                    product.name,
                                    { itemType },
                                );
                            }
                        ...
                        if (!matches) {
                            console.log(
                                "Fallback filter - product rejected:",
                                product.name,
    ...
    const handleUpdateStock = async (item) => {
        ...
        if (
            !update ||
            !update.inputValue ||
            parseFloat(update.inputValue) <= 0
        ) {
        ...
        try {
            ...
            if (response.status === "success") {
                ...
                setSuccess(
                    `${action} inventory successfully! Added ${calculatedValue.toFixed(3)} ${unit}`,
                );
                ...
                if (updatedInventory) {
                    setInventories((prev) =>
                        prev.map((prevItem) => {
                            ...
                            const prevProductIdStr = prevProductId
                            ...
                            if (prevProductIdStr === productIdStr) {
                                ...
                                const newCurrentStock =
                                    updatedInventory.stock_info
                                        ?.current_stock ??
                                    (updatedInventory.stock_info &&
                                    typeof updatedInventory.stock_info ===
                                        "object"
                                        ? updatedInventory.stock_info
                                              .current_stock
                                        : null) ??
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        >
                            ...
                            <option key="tinters" value="tinters">
                                Tinters
                            </option>
                            <option key="additive" value="additive">
                                Additive
                            </option>
                            <option key="binder" value="binder">
                                Binder
                            </option>
                            <option key="auxiliary" value="auxiliary">
                                Auxiliary
                            </option>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                    {loading ? (
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                    {displayItems.map((item, index) => {
                                        ...
                                        const standardQuantity =
                                            product.standard_quantity || 1;
                                        const standardQuantityUnit =
                                        ...
                                        if (inventory) {
                                            ...
                                            if (
                                                inventory.stock_info &&
                                                typeof inventory.stock_info ===
                                                    "object" &&
                                                inventory.stock_info
                                        ...
                                        const isUpdating =
                                            updating[productIdStr];

                                        return (
                                            <tr
                                                key={productIdStr}
                                                className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 ${!item.hasInventory ? "bg-yellow-50/50 dark:bg-yellow-900/10" : ""}`}
                                            >
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex items-center space-x-2">
                                                        ...
                                                        <div className="text-sm font-medium text-blue-600 dark:text-blue-400">
                                                            ...
                                                            0 ? (
                                                                <>
                                                                    {update.calculatedValue.toFixed(
                                                                        3,
                                                                    )}{" "}
                                                                    {stockUnit}
                                                                </>
                        ...
                        <div className="p-8 text-center text-gray-500 dark:text-gray-400">
                            No products found for selected branch and product
                            type
                        </div>
                    )}
                </div>
