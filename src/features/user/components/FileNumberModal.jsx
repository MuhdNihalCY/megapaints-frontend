...
const FileNumberModal = ({ 
  ...
  const validateUniqueness = async (value) => {
    ...
    try {
      const isUnique = await FileNumberService.validateFileNumber(value.trim());
      return isUnique;
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    if (!validateFormat(trimmedValue)) {
      setValidationError('Invalid file number format. Use numbers only (e.g., 100000) or with suffix (e.g., 100000.A)');
      return;
    }
    ...
    if (!isUnique) {
      setValidationError('This file number already exists. Please choose a different number.');
      return;
    }
    ...
    try {
      ...
      setValidationError('Error formatting file number. Please try again.');
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4">
        ...
        <div className="p-4">
          <div className="mb-4">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Enter a number (e.g., 100000) or with suffix (e.g., 100000.A)
            </p>
          ...
          {/* Preview */}
          {fileNumber && fileNumber.trim() && isValid && !validationError && (
        ...
        <div className="flex items-center justify-end space-x-3 p-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <button
            onClick={handleSave}
            disabled={!fileNumber || !fileNumber.trim() || !isValid || !!validationError || isLoading || isValidating}
