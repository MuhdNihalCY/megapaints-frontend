...
const ApiServiceExample = () => {
  ...
  const handleUserExample = async () => {
    ...
    try {
      ...
      const activitySummary = await userServices.user.getActivitySummary();
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-6 p-4 bg-gray-100 rounded-lg">
        ...
        <p><strong>User Type:</strong> {isAdmin ? 'Admin' : 'User'}</p>
        <p><strong>User:</strong> {user?.username || 'None'}</p>
        <p><strong>Role:</strong> {apiServiceFactory.getUserRole() || 'None'}</p>
      </div>
