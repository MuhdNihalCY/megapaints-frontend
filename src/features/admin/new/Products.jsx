        ...
        const fetchData = async () => {
            try {
                ...
                if (productsResponse.success) {
                    ...
                    throw new Error(
                        productsResponse.error || "Failed to fetch products",
                    );
                }
        ...
        try {
            ...
            if (editingProduct) {
                ...
                response = await apiRequest(
                    `/v1/product/${formData._id || formData.Product_Id}`,
                    {
                        method: "PUT",
                ...
                if (response.Status === true || response._id) {
                    ...
                    const updatedProducts = products.map((prod) =>
                        prod._id ===
                        (editingProduct._id || editingProduct.Product_Id)
                            ? { ...prod, ...formData }
                            : prod,
                    );
                ...
                if (response.Status === true || response._id) {
                    ...
                    const newProduct = response.Status
                        ? { ...formData }
                        : response;
        ...
        try {
            ...
            if (response.message) {
                ...
                setProducts(
                    products.filter(
                        (p) =>
                            p._id !== productId && p.Product_Id !== productId,
                    ),
                );
                setSnackbar({
                    open: true,
