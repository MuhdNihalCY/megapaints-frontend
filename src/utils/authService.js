...
class AuthService {
  ...
  async adminLogin(username, password) {
    try {
      ...
      if (!response.ok) {
        ...
        console.error('Admin login failed with status:', response.status, 'Error:', errorText);
      ...
      if (error.name === 'TypeError' && error.message.includes('Failed to fetch')) {
        return {
          success: false,
          message: 'Unable to connect to server. Please check if the backend is running and CORS is configured properly.'
        };
      }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          return {
            success: false,
            message: 'Registration endpoint not implemented yet. Please implement POST /api/auth/user/register in your backend.'
          };
        }
  ...
  async userLogin(username, password) {
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('User login failed with status:', response.status, 'Error:', errorText);
  ...
  async logout(type = 'user') {
    try {
      const endpoint = type === 'admin' ? '/auth/admin/logout' : '/auth/user/logout';
  ...
  async getCurrentProfile() {
    try {
      const userType = this.isAdmin() ? 'admin' : 'user';
      const endpoint = userType === 'admin' ? '/auth/admin/me' : '/auth/user/me';
