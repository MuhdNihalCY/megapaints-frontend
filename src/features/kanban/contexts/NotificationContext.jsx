    ...
    case ActionTypes.MARK_AS_CLICKED: {
      ...
      const notifications = state.notifications.map(n =>
        n._id === notificationId 
          ? { ...n, is_clicked: true, is_read: true, clicked_at: new Date().toISOString(), read_at: n.read_at || new Date().toISOString() } 
          : n
      );
      const unreadCount = notifications.filter(n => !n.is_read).length;
...
export function NotificationProvider({ children, currentUser }) {
  ...
  const createMentionNotificationsForComment = useCallback((comment, mentionedUserIds, author, card) => {
    const notifications = createMentionNotifications(comment, mentionedUserIds, author, card);
  ...
  const createAssignmentNotificationsForCard = useCallback((card, assignedUserIds, author) => {
    const notifications = createAssignmentNotifications(card, assignedUserIds, author);
    notifications.forEach(notification => {
      if (notification.recipient_id !== currentUser?.id) {
        addNotification(notification);
      }
