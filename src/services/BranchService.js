...
class BranchService {
    ...
    async getBranches(params = {}) {
        ...
        // API Call params });
        try {
            const response = await api.get(`${this.baseURL}/branches`, {
                params,
            });
    ...
    async searchBranches(query, params = {}) {
        ...
        try {
            ...
            const response = await api.get(`${this.baseURL}/branches/search`, {
                params: searchParams,
            });
            return this.handleResponse(response, endpoint);
        } catch (error) {
    ...
    async switchBranch(branchId) {
        ...
        // API Call branchId });
        try {
            const response = await api.post(`${this.baseURL}/branches/switch`, {
                branch_id: branchId,
            });
            return this.handleResponse(response, endpoint);
        } catch (error) {
    ...
    isValidBranchId(branchId) {
        return (
            branchId && branchId !== "default-branch-id" && branchId.length > 0
        );
    }
