...
const UserManagement = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            if (!adminServices) {
                setError(
                    "Admin services not available. Please log in as admin.",
                );
                setLoading(false);
                return;
            }
    ...
    const handleToggleStatus = async (user) => {
        ...
        try {
            ...
            setError(
                `Failed to ${user.is_active ? "deactivate" : "activate"} user: ${err.message}`,
            );
        }
    };
    ...
    const filteredAndSortedUsers = useMemo(() => {
        ...
        if (filterActive !== null) {
            filtered = filtered.filter(
                (user) => user.is_active === filterActive,
            );
        }
        ...
        if (filterDesignation) {
            filtered = filtered.filter(
                (user) => user.designation === filterDesignation,
            );
        }
