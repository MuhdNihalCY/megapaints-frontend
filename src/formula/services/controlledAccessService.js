...
export const verifyAccessKey = async (accessKey) => {
    try {
        ...
        const response = await api.post(
            "/v1/controlled_access/api/verify-access-key",
            {
        ...
        const message =
            ...
            response.data.msg ||
            (success
                ? "Access key verification successful"
                : "Access key verification failed");

        console.log("[ControlledAccess] Processed result:", {
            success,
            message,
        });
        ...
        console.error(
            "[ControlledAccess] Error response:",
            error.response?.data,
        );
