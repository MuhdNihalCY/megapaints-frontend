...
const BulkInventoryUpdate = ({ onClose, onSuccess, inline = false }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate all rows
    const invalidRows = updates.filter(update => 
      !update.branch_id || !update.product_id || update.quantity <= 0
    );
    
    if (invalidRows.length > 0) {
      setError('Please fill all required fields (Branch, Product, and Quantity > 0) for all rows');
      return;
    }
    ...
    try {
      ...
      const response = await adminServices.inventoryManagement.bulkUpdateInventory(formattedUpdates);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-4">
              <p className="text-sm text-blue-800 dark:text-blue-300">
                <strong>Instructions:</strong> Select branch and product for each row, choose operation (Add/Deduct/Update), and enter quantity. 
                You can add multiple rows to update multiple inventory items at once.
              </p>
            </div>
              ...
              <table className="w-full border-collapse">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  {updates.map((update, index) => {
                    const filteredProducts = getFilteredProducts(update.product_type);
                    return (
                      <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <td className="px-4 py-3">
                          <select
                            value={update.branch_id}
                            onChange={(e) => handleUpdateChange(index, 'branch_id', e.target.value)}
                        ...
                        <td className="px-4 py-3">
                          <select
                            ...
                            onChange={(e) => handleUpdateChange(index, 'product_id', e.target.value)}
                        ...
                        <td className="px-4 py-3">
                          <input
                            ...
                            onChange={(e) => handleUpdateChange(index, 'quantity', parseFloat(e.target.value) || 0)}
                            step="0.001"
                            min="0"
                            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white text-sm"
                            required
