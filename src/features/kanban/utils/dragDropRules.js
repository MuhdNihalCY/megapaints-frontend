 ...
 */
export const isMoveAllowed = (fromColumnId, toColumnId, fromSubColumnId, toSubColumnId) => {
  ...
  if (DRAG_DROP_RULES.RESTRICTED_COLUMNS.includes(fromSubColumnId) || 
...
export const validateDragStart = (card, user) => {
  ...
  if (card.subcolumnId && DRAG_DROP_RULES.RESTRICTED_COLUMNS.includes(card.subcolumnId)) {
    validation.canDrag = false;
...
export const validateDrop = (card, targetColumnId, targetSubColumnId, user) => {
  ...
  if (targetSubColumnId && DRAG_DROP_RULES.RESTRICTED_COLUMNS.includes(targetSubColumnId)) {
    validation.canDrop = false;
  ...
  if (!isMoveAllowed(card.columnId, targetColumnId, card.subcolumnId, targetSubColumnId)) {
    validation.canDrop = false;
 ...
 */
export const calculateNewPosition = (cards, targetColumnId, targetSubColumnId, dropIndex) => {
...
export const reorderCards = (cards, columnId, subColumnId) => {
  const columnCards = cards
    .filter(card => card.columnId === columnId && card.subcolumnId === subColumnId)
    .sort((a, b) => a.position - b.position);
...
export const canColumnAcceptDrops = (columnId, subColumnId) => {
  return !DRAG_DROP_RULES.RESTRICTED_COLUMNS.includes(columnId) &&
 ...
 */
export const handleDragEnd = (card, targetColumnId, targetSubColumnId, targetPosition, user) => {
