...
const Accessories = () => {
    ...
    const handleDeleteAccessory = async (accessory) => {
        if (
            !window.confirm(
                `Are you sure you want to delete accessory "${accessory.name}"? This action cannot be undone.`,
            )
        ) {
            return;
        }
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(
                (accessory) =>
                    ...
                    (accessory.code &&
                        accessory.code.toLowerCase().includes(searchLower)),
            );
        }
        if (filterActive !== null) {
            filtered = filtered.filter(
                (accessory) => accessory.is_active === filterActive,
            );
        }
        filtered.sort((a, b) => {
            let aValue, bValue;
                            ...
                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    0 ? (
                                        ...
                                        filteredAndSortedAccessories.map(
                                            (accessory) => (
                                                <tr
                                                    key={accessory._id}
                                                ...
                                                >
                                                    ...
                                                    <td className="px-3 sm:px-6 py-4 text-sm text-gray-700 dark:text-gray-300 max-w-xs">
                                                        <span className="line-clamp-2 block truncate" title={accessory.description || ""}>
