...
const Auxiliaries = () => {
    ...
    const handleDeleteAuxiliary = async (auxiliary) => {
        if (
            !window.confirm(
                `Are you sure you want to delete auxiliary "${auxiliary.name}"? This action cannot be undone.`,
            )
        ) {
            return;
        }
    ...
    const filteredAndSortedAuxiliaries = useMemo(() => {
        let filtered = [...auxiliaries];
        if (searchTerm) {
            const searchLower = searchTerm.toLowerCase();
            filtered = filtered.filter(
                (auxiliary) =>
                    (auxiliary.name &&
                        auxiliary.name.toLowerCase().includes(searchLower)) ||
                    (auxiliary.code &&
                        auxiliary.code.toLowerCase().includes(searchLower)),
            );
        }
        if (filterActive !== null) {
            filtered = filtered.filter(
                (auxiliary) => auxiliary.is_active === filterActive,
            );
        }
        filtered.sort((a, b) => {
            let aValue, bValue;
        ...
        return filtered;
    }, [auxiliaries, sortField, sortDirection, searchTerm, filterActive]);
                        ...
                        <div className="overflow-hidden">
                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    {filteredAndSortedAuxiliaries.length ===
                                    0 ? (
                                        <tr>
                                            <td
                                                colSpan="4"
                                        ...
                                        filteredAndSortedAuxiliaries.map(
                                            (auxiliary) => (
                                                <tr
                                                    key={auxiliary._id}
                                                ...
                                                >
                                                    ...
                                                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                                        <span className="font-semibold">
                                                            AED{" "}
                                                            {formatPrice(
                                                                auxiliary.base_price ??
                                                                    auxiliary.unit_price ??
                                                                    0,
                                                            )}
                                                        </span>
                                                    </td>
                                                    <td className="px-3 sm:px-6 py-4 text-sm text-gray-700 dark:text-gray-300 max-w-xs">
                                                        <span className="line-clamp-2 block truncate" title={auxiliary.description || ""}>
