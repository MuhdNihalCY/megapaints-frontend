...
const BranchForm = ({ branch = null, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    if (branch) {
      setFormData({
        ...
        phone: branch.phone || '',
        email: branch.email || '',
        manager_id: branch.manager?._id?._id || branch.manager?._id || '',
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await adminServices.businessManagement.getUsers({ limit: 100 });
  ...
  const validateForm = () => {
    ...
    if (!formData.address.country) {
      ...
      errors['address.country'] = `Country must be one of: ${GCC_COUNTRIES.join(', ')}`;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (branch) {
        ...
        response = await adminServices.businessManagement.updateBranch(branch._id, submitData);
      } else {
        // Create new branch
        response = await adminServices.businessManagement.createBranch(submitData);
      }

      if (response.status === 'success') {
        setSuccess(branch ? 'Branch updated successfully!' : 'Branch created successfully!');
        setTimeout(() => {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={handleInputChange}
                    className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-colors ${
                      validationErrors['address.country'] ? 'border-red-500 bg-red-50 dark:bg-red-900/10' : 'border-gray-300'
                    }`}
                  >
            ...
            <div>
              <label className="flex items-center">
                ...
                />
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Active Branch</span>
              </label>
            </div>
