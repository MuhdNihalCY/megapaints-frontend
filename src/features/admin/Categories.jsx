...
const Categories = () => {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const response = await adminServices.productCatalog.getCategories(params);
  ...
  const buildHierarchicalStructure = () => {
    ...
    categories.forEach(cat => {
      ...
      if (cat.parent_id) {
        const parent = categoryMap.get(cat.parent_id?._id || cat.parent_id);
        if (parent) {
  ...
  const handleDeleteCategory = async (category) => {
    if (!window.confirm(`Are you sure you want to delete category "${category.name}"? This will also delete all subcategories.`)) {
      return;
    }

    try {
      const adminServices = getAdminServices();
      const response = await adminServices.productCatalog.deleteCategory(category._id);
  ...
  const renderCategoryRow = (category, level = 0) => {
    ...
    return (
      <div key={category._id}>
        <div
          className={`flex items-center px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border-b border-gray-200 dark:border-gray-700 ${
          ...
          <div className="flex items-center space-x-3">
            <div className="flex-1">
              <div className="flex items-center space-x-2">
                ...
                {category.parent_id && (
                  <span className="text-xs text-gray-500 dark:text-gray-400">
                    Parent: {category.parent_id.name || category.parent_id._id}
                  </span>
                )}
              </div>
              {category.description && (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
        {/* Table Header */}
        <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 px-6 py-3 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <Layers className="w-5 h-5 text-gray-600 dark:text-gray-400" />
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                {viewMode === 'hierarchical' ? 'Hierarchical Categories' : 'All Categories'}
              </h3>
            </div>
