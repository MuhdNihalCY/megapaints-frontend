...
class BusinessManagementService {
  ...
  async getBranchById(branchId) {
    return await this.adminApi.apiRequest(`/admin/business/branches/${branchId}`);
  }
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          inventorySummary: branch.data.branch.inventory_summary || {},
          lowStockProducts: branch.data.branch.low_stock_products || 0
        }
      };
    } catch (error) {
  ...
  async deleteUser(userId) {
    return await this.adminApi.apiRequest(`/admin/business/users/${userId}`, {
      method: 'DELETE'
    });
  }
  ...
  async resetUserPassword(userId, newPassword) {
    return await this.adminApi.apiRequest(`/admin/business/users/${userId}/reset-password`, {
      method: 'POST',
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          inactiveUsers: (users.data.pagination?.total || 0) - (activeUsers.data.pagination?.total || 0)
        }
      };
    } catch (error) {
    ...
    try {
      ...
      return {
        ...
        data: {
          ...
          activeBranches: branchStats.filter(b => b.is_active).length
        }
      };
    } catch (error) {
  ...
  async exportBranchesToCSV(options = {}) {
    try {
      const branches = await this.getBranches({ limit: 1000, ...options });
  ...
  convertUsersToCSV(users) {
    ...
    const headers = [
      ...
      'company', 'designation', 'roles', 'permissions', 'is_active', 'last_login'
    ];
  ...
  convertBranchesToCSV(branches) {
    ...
    const headers = [
      'name', 'code', 'phone', 'email', 'address', 'is_active', 'createdAt'
    ];
