...
const Tinters = () => {
    ...
    const fetchTinters = async () => {
        try {
            ...
            const response =
                await adminServices.productCatalog.getProducts(params);
    ...
    const handleDeleteTinter = async (tinter) => {
        if (
            !window.confirm(
                `Are you sure you want to delete tinter "${tinter.name}"? This action cannot be undone.`,
            )
        ) {
            return;
        }
        ...
        try {
            const adminServices = getAdminServices();
            const response = await adminServices.productCatalog.deleteProduct(
                tinter._id,
            );
    ...
    const filteredAndSortedTinters = useMemo(() => {
        ...
        if (filterActive !== null) {
            filtered = filtered.filter(
                (tinter) => tinter.is_active === filterActive,
            );
        }
