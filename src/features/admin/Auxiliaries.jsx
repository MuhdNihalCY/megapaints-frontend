...
const Auxiliaries = () => {
  ...
  const filteredAndSortedAuxiliaries = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(auxiliary => 
        (auxiliary.name && auxiliary.name.toLowerCase().includes(searchLower)) ||
        (auxiliary.code && auxiliary.code.toLowerCase().includes(searchLower))
      );
    }
    if (filterActive !== null) {
      filtered = filtered.filter(auxiliary => auxiliary.is_active === filterActive);
    }
    filtered.sort((a, b) => {
      let aValue, bValue;
      switch (sortField) {
            ...
            <div className="overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredAndSortedAuxiliaries.length === 0 ? (
                    <tr>
                ...
                filteredAndSortedAuxiliaries.map((auxiliary) => (
                  <tr key={auxiliary._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white hidden md:table-cell">
                      <div className="truncate block max-w-[150px]">{auxiliary.type || '-'}</div>
                    ...
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white hidden lg:table-cell">
                      <div className="flex items-center">
                        <span className="font-semibold">AED {auxiliary.unit_price?.toFixed(2) || '0.00'}</span>
