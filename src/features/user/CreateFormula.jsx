...
import { fetchBindersByIds } from "../../formula/services/binderService";
...
const CreateFormula = () => {
    ...
    function normalizeTints(list) {
        ...
        const out = safe.map((t, idx) => {
            ...
            return {
                ...
                Product_Density: Number.isFinite(productDensity)
                    ? productDensity
                    : 0, // Product density
        ...
        (async () => {
            ...
            try {
                ...
                const rawAdds = Array.isArray(data?.additives)
                    ? data.additives
                    : [];
                const rawBinds = Array.isArray(data?.binders)
                    ? data.binders
                    : [];

                // Debug: Log binder data received
                if (process.env.NODE_ENV === 'development') {
                    console.log('[Masters Data] Binders received:', {
                        count: rawBinds.length,
                        firstBinder: rawBinds[0] ? {
                            Binder_Id: rawBinds[0].Binder_Id,
                            _id: rawBinds[0]._id,
                            Binder_Name: rawBinds[0].Binder_Name
                        } : null
                    });
                }
                ...
                if (!cancelled) {
                    ...
                    setSubCategoryOptions(
                        Array.isArray(initialSubs) && initialSubs.length
                            ? initialSubs
                            : fallbackSubs,
                    );
    ...
    useEffect(() => {
        const effectDebug = {
            ...
            hasBinderConfig: !!binderConfigBySubCategory[subCategory],
            productsKeys: Object.keys(productsBySubCategory).slice(0, 5),
            binderKeys: Object.keys(binderConfigBySubCategory).slice(0, 5),
        };
        console.log(
            "[useEffect Subcategory] Triggered:",
            JSON.stringify(effectDebug, null, 2),
        );

        // Auto-select binders even if products aren't loaded yet
    ...
    // Dedicated useEffect for binder auto-selection (for extra reliability)
    useEffect(() => {
        if (subCategory && binderConfigBySubCategory) {
            console.log(
                "[Dedicated Binder Effect] Running auto-selection for:",
                subCategory,
            );
            autoSelectBindersForSubcategory(subCategory);
        }
    ...
    const handleProductSearch = (tintId, searchTerm) => {
        ...
        const filtered = availableProducts.filter((product) => {
            const matchesSearch =
                ...
                product.Abbreviation?.toLowerCase().includes(
                    searchTerm.toLowerCase(),
                ) ||
            ...
            const notAlreadySelected = !tints.some(
                (tint) =>
    ...
    const selectProduct = (tintId, product) => {
        ...
        updateTint(
            tintId,
            "Product_Density",
            Number(product.Product_Density || 0),
        );
    ...
    const autoSelectBindersForSubcategory = (subCategoryId) => {
        ...
        const debugInfo = {
            subCategoryId,
            availableConfigs: Object.keys(binderConfigBySubCategory),
            configExists: !!binderConfigBySubCategory[subCategoryId],
            totalConfigs: Object.keys(binderConfigBySubCategory).length,
        };
        console.log(
            "[Binder Auto-Select] Called with:",
        ...
        const subcategoryConfig =
            ...
            Object.values(binderConfigBySubCategory).find(
                (config) =>
                    ...
                    String(config.SubCategory_Id) === String(subCategoryId) ||
                    String(config._id) === String(subCategoryId),
            );

        const lookupResult = {
            found: !!subcategoryConfig,
            binder1: subcategoryConfig?.Binder1,
        ...
        if (subcategoryConfig) {
            ...
            const binder1Id =
                ...
                "";
            const binder2Id =
                subcategoryConfig.Binder2 ||
                subcategoryConfig.Products?.Binder2 ||
                "";

            const extractedInfo = {
                binder1Id,
                binder2Id,
                source: subcategoryConfig.Products
                    ? "Products object"
                    : "direct properties",
                binder1Name: subcategoryConfig.Binder1Name,
                binder2Name: subcategoryConfig.Binder2Name,
            };
            console.log(
                "[Binder Auto-Select] Extracted binder IDs:",
                ...
                fetchBindersByIds(idsToFetch)
                    .then(fetchedBinders => {
                        if (fetchedBinders.length > 0) {
                            console.log('[Binder Fallback] Fetched binders:', fetchedBinders);
                            setRawBinders(prev => {
                                // Merge with existing, avoiding duplicates
                                const existingIds = new Set(prev.map(b => b.Binder_Id));
                                const newBinders = fetchedBinders.filter(
                                    b => !existingIds.has(b.Binder_Id)
                                );
                                return [...prev, ...newBinders];
                            });
                        } else {
                            console.warn('[Binder Fallback] No binders found for IDs:', idsToFetch);
                        }
                    })
                    .catch(error => {
    ...
    const selectedBinderConfig = useMemo(() => {
        ...
        const binder2Data = rawBinders.find(
            (b) => String(b.Binder_Id) === String(selectedBinder2),
        );

        // Debug: Log binder lookup
        if (process.env.NODE_ENV === 'development' && (selectedBinder1 || selectedBinder2)) {
            console.log('[Binder Lookup] Searching for binders:', {
                selectedBinder1,
                selectedBinder2,
                rawBindersCount: rawBinders.length,
                binder1Found: !!binder1Data,
                binder2Found: !!binder2Data,
        ...
        const config = {
            ...
            Binder1Name:
                ...
                binder1Data?.name ||
                cfg?.Binder1Name ||
                `Binder ${selectedBinder1}`,
            Binder2Name:
                binder2Data?.Binder_Name ||
                binder2Data?.name ||
                cfg?.Binder2Name ||
    ...
    const finalTotals = useMemo(
        () =>
            computeFinalTotals(
                {
                    totalGrams: tinterTotals.totalGrams,
                    totalVolumeL: tinterTotals.totalVolumeL,
                },
                {
                    totalBinderGrams: binderTotals.totalBinderGrams,
                    totalBinderVolumeL: binderTotals.totalBinderVolumeL,
                },
                {
                    totalAdditiveGrams: additiveTotals.totalAdditiveGrams,
                    totalAdditiveVolumeL: additiveTotals.totalAdditiveVolumeL,
                },
            ),
    ...
    const save = async (saveAsNewVersion = false) => {
        ...
        const payload = {
            ...
            totals: {
                ...
                binder: {
                    grams: binderTotals.totalBinderGrams,
                    volumeL: binderTotals.totalBinderVolumeL,
                },
                additive: {
                    grams: additiveTotals.totalAdditiveGrams,
                    volumeL: additiveTotals.totalAdditiveVolumeL,
                },
                final: {
                ...
                <div className="grid grid-cols-12 gap-6">
                        ...
                        {process.env.NODE_ENV === "development" && (
                            <div className="bg-yellow-50 dark:bg-yellow-900 p-4 rounded shadow mb-4">
                                ...
                                <div className="text-xs space-y-1 text-yellow-700 dark:text-yellow-300">
                                    ...
                                    <div>
                                        Selected Tinters:{" "}
                                        {tints.filter((t) => t.code).length}
                                    </div>
                                    <div>
                                        Available in Subcategory:{" "}
                                        {
                                            tints.filter(
                                                (t) =>
                                                    t.code &&
                                    ...
                                    <div>
                                        Current Additives: {additives.length}
                                    </div>
                                    <div>
                                        Selected Additive:{" "}
                                        {selectedAdditiveId || "None"}
                                    </div>
                                    <div>
                                        Additive Percentage:{" "}
                                        {additivePercentageInput || "0"}%
                                    </div>
                                    <div>
                                        Tinter Total Grams:{" "}
                                        {tinterTotals.totalGrams.toFixed(2)}
                                    </div>
                                    <div>
                                        Binder Total Grams:{" "}
                                        {binderTotals.totalBinderGrams.toFixed(
                                            2,
                                        )}
                                    </div>
                                    <div>
                                        Base Mass for Additives:{" "}
                                        {(
                                            tinterTotals.totalGrams +
                                            binderTotals.totalBinderGrams
                                        ).toFixed(2)}
                                    </div>
                                    <div>
                                        Additive Total Grams:{" "}
                                        {additiveTotals.totalAdditiveGrams.toFixed(
                                            2,
                                        )}
                                    </div>
                                    <div>
                                        Available Binders: {rawBinders.length}
                                    </div>
                                    <div>
                                        Binder Config:{" "}
                                        {selectedBinderConfig ? "Yes" : "No"}
                                    </div>
                                    <div>
                                        Auto-Selected Binder1:{" "}
                                        {selectedBinder1Id || "None"}
                                    </div>
                                    <div>
                    ...
                    <div className="col-span-10">
                      ...
                      Binder names and calculations are automatically selected based on the chosen subcategory configuration.
                                    ...
                                    {/* Show binder auto-selection status in development mode */}
                                    {process.env.NODE_ENV === "development" && (
                                        <div className="text-xs text-blue-300 mb-2 bg-blue-900 bg-opacity-30 p-2 rounded">
                                            Auto-Selected: Binder1=
                                            {selectedBinder1Id || "None"} |
                                            Binder2=
                                            {selectedBinder2Id || "None"}
                                                    ...
                                                    if (selectedAdditiveId) {
                                                        ...
                                                        if (additive) {
                                                            ...
                                                            if (
                                                                ...
                                                                console.log(
                                                                    "[Dynamic Data] Additive percentage updated:",
                                                                    {
                                                                        additiveId:
                                                                            selectedAdditiveId,
                                                                        additiveName:
                                                                ...
                                                                setAdditives(
                                                                    (prev) => {
                                                                        ...
                                                                        if (
                                                                            ...
                                                                            console.log(
                                                                                "[Dynamic Data] Additive added with percentage:",
                                                                                {
                                                                                    additiveId:
                                                                                        selectedAdditiveId,
                                                                                    additiveName:
