...
const Orders = () => {
    ...
    const fetchOrders = async () => {
        try {
            ...
            const response = await adminServices.orderManagement.getOrders(
                activeTab,
                filterParams,
                paginationParams,
            );
    ...
    const handleDeleteOrder = async (order) => {
        ...
        try {
            const adminServices = getAdminServices();
            const response = await adminServices.orderManagement.deleteOrder(
                order._id,
                order.order_type,
            );
    ...
    const handleStatusChange = async (order, newStatus) => {
        try {
            const adminServices = getAdminServices();
            const response =
                await adminServices.orderManagement.updateOrderStatus(
                    order._id,
                    order.order_type,
                    newStatus,
                );
    ...
    const getStatusBadge = (status) => {
        ...
        return (
            <span
                className={`px-2 py-1 text-xs font-semibold rounded-full ${statusColors[status] || statusColors.pending}`}
            >
