  ...
  switch (type) {
    case CUSTOM_FIELD_TYPES.TEXT:
      ...
      if (options.maxLength && value.length > options.maxLength) {
        return { valid: false, error: `Max ${options.maxLength} characters` };
      }
      return { valid: true };
    ...
    case CUSTOM_FIELD_TYPES.DROPDOWN:
      if (options.allowMultiple && Array.isArray(value)) {
        const optionLabels = value.map(v => {
          const option = options.options.find(opt => opt.value === v);
...
export const updateCustomFieldValue = (card, fieldId, value, userId) => {
  ...
  if (existingFieldIndex >= 0) {
    ...
    updatedFields[existingFieldIndex] = createCustomFieldValue(fieldId, value, userId);
    return { ...card, customFields: updatedFields };
  } else {
    // Add new
    return {
      ...card,
      customFields: [...existingFields, createCustomFieldValue(fieldId, value, userId)]
    };
  }
};
...
export const removeCustomFieldValue = (card, fieldId) => {
  return {
    ...card,
    customFields: (card.customFields || []).filter(cf => cf.fieldId !== fieldId)
  };
};
...
export const DEFAULT_CUSTOM_FIELDS = [
  ...
  },
  {
    ...
    options: {
      options: [
        { value: 'not-started', label: 'Not Started', color: '#b3bac5' },
        { value: 'in-progress', label: 'In Progress', color: '#00c2e0' },
