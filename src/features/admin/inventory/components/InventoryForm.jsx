...
const InventoryForm = ({ inventory = null, onClose, onSuccess }) => {
        ...
        if (inventory && products.length > 0) {
            ...
            if (product) {
                setSelectedProductType(
                    product.item_type || product.product_type || "product",
                );
            }
            setFormData({
        ...
        try {
            ...
            if (response.status === "success") {
                ...
                if (selectedProductType) {
                    setFilteredProducts(
                        items.filter(
                            (p) => p.item_type === selectedProductType,
                        ),
                    );
                } else {
    ...
    useEffect(() => {
        if (products.length > 0) {
            if (selectedProductType) {
                setFilteredProducts(
                    products.filter((p) => p.item_type === selectedProductType),
                );
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (inventory) {
                const branchId =
                    inventory.branch?._id || inventory.branch?._id?._id;
                const productId =
                    inventory.product?._id || inventory.product?._id?._id;
                ...
                if (stockOperationMode === "add") {
                    ...
                    const inputValue =
                        stockQuantityToAdd === ""
                            ? 0
                            : parseFloat(stockQuantityToAdd) || 0;
                    if (inputValue <= 0) {
                    ...
                    const response =
                        await adminServices.inventoryManagement.addStock(
                            branchId,
                            productId,
                            calculatedQuantity,
                        );
                    ...
                    if (response.status === "success") {
                        setSuccess(
                            `Stock added successfully! Added ${calculatedQuantity.toFixed(3)} ${formData.stock_info.unit} (${inputValue} units × ${standardQuantity.toFixed(3)})`,
                        );
                        setTimeout(() => {
                            onSuccess && onSuccess();
                            onClose && onClose();
                    ...
                    const response =
                        await adminServices.inventoryManagement.updateInventory(
                            branchId,
                            productId,
                            inventoryData,
                        );
                ...
                const response =
                    await adminServices.inventoryManagement.createInventory({
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="space-y-4">
                            ...
                            <SearchableSelect
                                ...
                                disabled={
                                    !!inventory || filteredProducts.length === 0
                                }
                                searchKey="name"
                                displayKey="name"
                                secondaryKey="code"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <>
                                        <div>
                                            ...
                                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                                Current Stock:{" "}
                                                {formData.stock_info.current_stock.toFixed(
                                                    3,
                                                )}{" "}
                                                {formData.stock_info.unit}
                                            </p>
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        value={
                                            formData.stock_info.minimum_stock
                                        }
                                        onChange={handleInputChange}
                                        step="0.001"
                                        min="0"
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        value={
                                            formData.stock_info.maximum_stock
                                        }
                                        onChange={handleInputChange}
                                        step="0.001"
                                        min="0"
