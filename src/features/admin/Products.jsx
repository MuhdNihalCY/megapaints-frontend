...
const Products = () => {
    ...
    const fetchSubCategories = async () => {
        ...
        try {
            const adminServices = getAdminServices();
            const response =
                await adminServices.productCatalog.getSubcategories(
                    filterCategory,
                    { limit: 100 },
                );
            if (response.status === "success") {
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                    ...
                    >
                        ...
                        {subCategories.map((subCategory, index) => (
                            <option
                                key={subCategory._id || `subcategory-${index}`}
                                        ...
                                        filteredAndSortedProducts.map(
                                            (product) => {
                                                const inventory =
                                                    ...
                                                    {};
                                                const hasLowStock =
                                                    inventory.low_stock_branches >
                                                    0;
                                                const hasOutOfStock =
                                                    inventory.out_of_stock_branches >
                                                    0;
                                                const typeColor =
                                                    getProductTypeColor(
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                                                            <span
                                                                className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-gradient-to-r ${typeColor} text-white`}
                                                            >
                                                                {getProductTypeLabel(
                                                                    product.product_type,
                                                                )}
