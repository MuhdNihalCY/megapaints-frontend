...
const Accessories = () => {
  ...
  const handleDeleteAccessory = async (accessory) => {
    if (!window.confirm(`Are you sure you want to delete accessory "${accessory.name}"? This action cannot be undone.`)) {
      return;
    }
  ...
  const filteredAndSortedAccessories = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(accessory => 
        (accessory.name && accessory.name.toLowerCase().includes(searchLower)) ||
        (accessory.code && accessory.code.toLowerCase().includes(searchLower))
      );
    }
    if (filterActive !== null) {
      filtered = filtered.filter(accessory => accessory.is_active === filterActive);
    }
    filtered.sort((a, b) => {
      let aValue, bValue;
      switch (sortField) {
    ...
    return filtered;
  }, [accessories, sortField, sortDirection, searchTerm, filterActive]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Accessories Management</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Manage paint accessories and components
          </p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <select
            ...
            className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
          >
                  ...
                  {filteredAndSortedAccessories.length === 0 ? (
                    ...
                    <p className="text-gray-400 dark:text-gray-500 text-sm mt-2">
                      {searchTerm || filterActive !== null
                        ...
                        : 'Get started by adding a new accessory'}
                    </p>
                  </td>
                </tr>
              ) : (
                filteredAndSortedAccessories.map((accessory) => (
                  <tr key={accessory._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <div className="flex items-center">
                        <div className="min-w-0">
                          ...
                          <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 truncate">
                            ID: {accessory._id?.toString().slice(-8) || accessory.id}
                          </div>
                    ...
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white hidden md:table-cell">
                      <div className="truncate block max-w-[150px]">{accessory.category || '-'}</div>
                    ...
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white hidden lg:table-cell">
                      <div className="flex items-center">
                        <span className="font-semibold">AED {accessory.unit_price?.toFixed(2) || '0.00'}</span>
          ...
          <div className="bg-gray-50 dark:bg-gray-700/50 px-3 sm:px-6 py-4 border-t border-gray-200 dark:border-gray-700">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-xs sm:text-sm text-gray-700 dark:text-gray-300">
                ...
                of <span className="font-medium">{pagination.total}</span> accessories
              </div>
