...
const Additives = () => {
  ...
  const fetchAdditives = async () => {
    try {
      ...
      if (response.status === 'success') {
        ...
        setError('Failed to fetch additives');
      }
    } catch (err) {
      console.error('Failed to fetch additives:', err);
      setError(err.message || 'Failed to fetch additives');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteAdditive = async (additive) => {
    if (!window.confirm(`Are you sure you want to delete additive "${additive.name}"? This action cannot be undone.`)) {
      return;
    }
    ...
    try {
      ...
      if (response.status === 'success') {
        ...
        setError('Failed to delete additive');
      }
    } catch (err) {
      console.error('Failed to delete additive:', err);
  ...
  const filteredAndSortedAdditives = useMemo(() => {
    let filtered = [...additives];
    
    if (searchTerm) {
      const searchLower = searchTerm.toLowerCase();
      filtered = filtered.filter(additive => 
        (additive.name && additive.name.toLowerCase().includes(searchLower)) ||
        (additive.code && additive.code.toLowerCase().includes(searchLower))
      );
    }
    
    if (filterActive !== null) {
      filtered = filtered.filter(additive => additive.is_active === filterActive);
    }
    ...
    return filtered;
  }, [additives, sortField, sortDirection, searchTerm, filterActive]);

  if (loading && additives.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center space-y-4">
          <Activity className="w-8 h-8 text-blue-500 animate-spin" />
          <p className="text-gray-600 dark:text-gray-400">Loading additives...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Additives Management</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Manage paint additives and components
          </p>
        </div>
            ...
            <div className="overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredAndSortedAdditives.length === 0 ? (
                    <tr>
                      <td colSpan="5" className="px-3 sm:px-6 py-12 text-center">
                    <Beaker className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                    <p className="text-gray-500 dark:text-gray-400 text-lg font-medium">No additives found</p>
                    <p className="text-gray-400 dark:text-gray-500 text-sm mt-2">
                      {searchTerm || filterActive !== null
                        ? 'Try adjusting your filters'
                        : 'Get started by adding a new additive'}
                    </p>
                  </td>
                </tr>
              ) : (
                filteredAndSortedAdditives.map((additive) => (
                  <tr key={additive._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white hidden md:table-cell">
                      <div className="truncate block max-w-[150px]">{additive.category || '-'}</div>
                    ...
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white hidden lg:table-cell">
                      <div className="flex items-center">
                        <span className="font-semibold">AED {additive.unit_price?.toFixed(2) || '0.00'}</span>
                    ...
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end space-x-1 sm:space-x-2">
                        ...
                        <button
                          onClick={() => handleDeleteAdditive(additive)}
          ...
          <div className="bg-gray-50 dark:bg-gray-700/50 px-3 sm:px-6 py-4 border-t border-gray-200 dark:border-gray-700">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-xs sm:text-sm text-gray-700 dark:text-gray-300">
                ...
                of <span className="font-medium">{pagination.total}</span> additives
              </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-700/50 px-3 sm:px-6 py-3 border-t border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div className="flex items-center text-sm text-gray-700 dark:text-gray-300">
                ...
                <span className="font-medium">{pagination.total}</span> total additives
              </div>
            </div>
          </div>
        )}
