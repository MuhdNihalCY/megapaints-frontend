                    ...
                    >
                        ...
                        >
                            ...
                            <Chip
                                ...
                                size="small"
                                variant={
                                    column.is_active ? "filled" : "outlined"
                                }
                                sx={
                                    !column.is_active
                                        ? {
                            ...
                            >
                                ...
                                >
                                    ...
                                    <Typography
                                        ...
                                        sx={{ mb: 2 }}
                                    >
                                        Select designations to automatically
                                        generate sub-columns from users with
                                        those designations
                                    </Typography>
                                    <FormControl fullWidth size="small">
                                        ...
                                        <Select
                                            multiple
                                            value={
                                                column.sub_column_designations ||
                                                []
                                            }
                                            onChange={(e) => {
                                                ...
                                                onEdit({
                                                    ...column,
                                                    sub_column_designations:
                                                        selectedDesignations,
                                                });
                                            }}
                                            input={
                                                <OutlinedInput label="Select Designations" />
                                            }
                                            sx={{
                                                "& .MuiSelect-select": {
                                                    color: theme.palette.text
                                    ...
                                    {(column.sub_column_designations || [])
                                        ...
                                        >
                                            Sub-columns will be generated from
                                            users with:{" "}
                                            {column.sub_column_designations.join(
                                                ", ",
                                            )}
                                        </Typography>
                                    )}
                                ...
                                >
                                    Static sub-columns appear first, followed by
                                    dynamic user-based sub-columns
                                </Typography>

                                {column.sub_columns &&
                                column.sub_columns.length > 0 ? (
                                    <Box
                                        sx={{
                                            display: "flex",
                                    ...
                                    >
                                        No sub-columns. Click "Add Sub-column"
                                        to add one.
                                    </Typography>
                                )}
                            </Box>
...
const KanbanColumns = () => {
        ...
        try {
            ...
            if (err.response?.status === 401) {
                ...
                const errorMessage =
                    ...
                    err.message ||
                    "Failed to load column template";
                setError(errorMessage);
    ...
    const handleEditColumn = (updatedColumn) => {
        const newColumns = template.columns.map((col) =>
            (col._id || col.id) === (updatedColumn._id || updatedColumn.id)
                ? updatedColumn
                : col,
        );
        setTemplate({ ...template, columns: newColumns });
    ...
    const handleDeleteColumn = (columnToDelete) => {
        ...
        const newColumns = template.columns.filter(
            (col) =>
                (col._id || col.id) !==
                (columnToDelete._id || columnToDelete.id),
        );
        // Reorder positions
        newColumns.forEach((col, index) => {
            col.position = index;
    ...
    const handleEditSubColumn = (parentColumn, subColumn, updates) => {
        const updatedSubColumns = (parentColumn.sub_columns || []).map((sub) =>
            (sub.id || sub._id) === (subColumn.id || subColumn._id)
                ? { ...sub, ...updates }
                : sub,
        );
        const updatedColumn = {
    ...
    const handleDeleteSubColumn = (parentColumn, subColumnToDelete) => {
        const updatedSubColumns = (parentColumn.sub_columns || []).filter(
            (sub) =>
                (sub.id || sub._id) !==
                (subColumnToDelete.id || subColumnToDelete._id),
        );
        // Reorder positions
    ...
    const handleSave = async () => {
        try {
            ...
            let response;
            if (template._id) {
                response = await columnTemplateApi.update(
                    template._id,
                    templateData,
                );
            } else {
    ...
    const handleApplyToAllBoards = async () => {
        try {
            setSaving(true);
            const response = await columnTemplateApi.applyToAllBoards(
                template._id,
            );
            if (response.data?.status === "success") {
                ...
                const totalCards =
                    (migrationData?.total_cards_migrated || 0) +
                    (migrationData?.total_cards_orphaned || 0);
                ...
                if (migrationData?.total_cards_orphaned > 0) {
                    toast(
                        `⚠️ ${migrationData.total_cards_orphaned} cards were moved to default column because their original columns were deleted.`,
                        {
                            icon: "⚠️",
                            duration: 8000,
                        },
            ...
            >
                ...
                {!error && (
                    <Alert severity="info" sx={{ mb: 2 }}>
                        No template found. Click the button below to create a
                        default template.
                    </Alert>
                )}
                <Button
        ...
        >
            ...
            >
                ...
                <DialogContent>
                    <Alert severity="warning" sx={{ mb: 2 }}>
                        ...
                        <Typography
                            ...
                            sx={{ color: theme.palette.text.primary }}
                        >
                            This will update all existing boards to use this
                            column template. Existing cards will be
                            automatically migrated to matching columns.
                        </Typography>
                    </Alert>
                    ...
                    <Box sx={{ mt: 2 }}>
                        ...
                        >
                            <ul style={{ margin: 0, paddingLeft: 20 }}>
                                <li>
                                    All boards will get the new column structure
                                    from this template
                                </li>
                                <li>
                                    Cards will be automatically moved to columns
                                    with matching names
                                </li>
                                <li>
                                    If a column is deleted, its cards will be
                                    moved to the first available column
                                </li>
                                <li>
                                    Sub-columns will be preserved if they exist
                    ...
                    <Alert severity="info" sx={{ mt: 2 }}>
                        <Typography
                            ...
                            sx={{ color: theme.palette.text.primary }}
                        >
                            A detailed migration report will be shown after the
                            operation completes.
                        </Typography>
                    </Alert>
                </DialogContent>
                <DialogActions>
                    ...
                    <Button
                        onClick={handleApplyToAllBoards}
                        variant="contained"
                        color="primary"
                        disabled={saving}
                    >
                        ...
                        <Box>
                            ...
                            <Box sx={{ mb: 3 }}>
                                ...
                                >
                                    ...
                                    <Chip
                                        ...
                                        color={
                                            migrationReport.total_cards_orphaned >
                                            0
                                                ? "warning"
                                                : "default"
                                        }
                            ...
                            {migrationReport.total_cards_orphaned > 0 && (
                                <Alert severity="warning" sx={{ mb: 2 }}>
                                    ...
                                    >
                                        {migrationReport.total_cards_orphaned}{" "}
                                        cards were moved to the default column
                                        because their original columns were
                                        deleted. Please review these cards.
                                    </Typography>
                                </Alert>
                            )}

                            {migrationReport.board_reports &&
                                migrationReport.board_reports.length > 0 && (
                                    <Box>
                                        <Typography
                                            variant="h6"
                                            sx={{
                                        ...
                                        {migrationReport.board_reports.map(
                                            (report, index) => (
                                                <Paper
                                                    key={index}
                                                    elevation={0}
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <Chip
                                                            label={`${report.cards_migrated} migrated`}
                                                            size="small"
                                                            color="success"
                                                        />
                                                        <Chip
                                                        ...
                                                        />
                                                        {report.columns_deleted >
                                                            0 && (
                                                            <Chip
                                                                label={`${report.columns_deleted} columns deleted`}
                                                                size="small"
                                                                color="error"
                                                            />
                                                        )}
                                                    </Box>
                                                    {report.deleted_column_names &&
                                                        report
                                                            ...
                                                            >
                                                                Deleted columns:{" "}
                                                                {report.deleted_column_names.join(
                                                                    ", ",
                                                                )}
                                                            </Typography>
                                                        )}
