...
class KanbanBoardService {
    ...
    async getBoardsByBranch(branchId) {
        return await this.apiRequest(
            `/board/branch?branchId=${encodeURIComponent(branchId)}`,
        );
    }
    ...
    async getBranchBoardData(params = {}) {
        try {
            ...
            const boardData = await this.apiRequest(
                `/board/branch?${queryString}`,
            );

            // Normalize response shape - handle both direct stats and nested data
            const normalizedStats =
                boardData?.data?.stats || boardData?.stats || {};
    ...
    async moveCard(cardId, moveData) {
        return await this.apiRequest(
            `/card/${encodeURIComponent(cardId)}/move`,
            {
                method: "POST",
    ...
    async archiveCard(cardId) {
        return await this.apiRequest(
            `/card/${encodeURIComponent(cardId)}/archive`,
            {
                method: "POST",
            },
        );
    ...
    async restoreCard(cardId) {
        return await this.apiRequest(
            `/card/${encodeURIComponent(cardId)}/restore`,
            {
                method: "POST",
            },
        );
    ...
    async duplicateCard(cardId, duplicateData = {}) {
        return await this.apiRequest(
            `/card/${encodeURIComponent(cardId)}/duplicate`,
            {
                method: "POST",
    ...
    async getCardComments(cardId, options = {}) {
        ...
        return await this.apiRequest(
            `/comment/card/${encodeURIComponent(cardId)}?${queryString}`,
        );
    }
    ...
    async addComment(cardId, commentData) {
        return await this.apiRequest(
            `/comment/card/${encodeURIComponent(cardId)}`,
            {
                method: "POST",
