  ...
  if (id && typeof id === 'object') {
    ...
    if (id.toString && typeof id.toString === 'function') {
      try {
        ...
        if (str && str !== '[object Object]' && str.length > 0 && str.length < 50 && str.length >= 12) {
          return str.trim();
        }
      } catch (e) {
        // Continue to other methods
    ...
    if (id._id) {
      ...
      if (nestedId && nestedId !== '[object Object]' && nestedId.length > 0) return nestedId;
    }
    ...
    const str = String(id);
    if (str && str !== '[object Object]' && str !== 'undefined' && str !== 'null' && str.length > 0 && str.length < 50) {
      return str.trim();
    }
    return '';
  }
...
const ProductForm = ({ product = null, defaultProductType = null, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.subcategories && Array.isArray(product.subcategories) && product.subcategories.length > 0) {
        subcategoryIds = product.subcategories.map(sub => normalizeId(sub._id || sub));
      ...
      setFormData({
        ...
        standard_quantity_unit: product.standard_quantity_unit || product.unit || 'kg',
        voc: product.voc || '',
  ...
  useEffect(() => {
    ...
    if (!categoryIdStr || categoryIdStr === '[object Object]' || categoryIdStr === 'undefined' || categoryIdStr === 'null' || categoryIdStr.trim() === '') {
      setSubcategories([]);
  ...
  const fetchSubcategories = async (categoryIdParam = null) => {
    try {
      ...
      let categoryId = categoryIdParam !== null ? categoryIdParam : formData.category_id;
      if (!categoryId) {
        setSubcategories([]);
        return;
      }
      ...
      if (typeof categoryId === 'string') {
        ...
        if (categoryId.toString && typeof categoryId.toString === 'function') {
          try {
            parentId = categoryId.toString();
          } catch (e) {
            // If toString fails, try other methods
            if (categoryId._id) {
              parentId = typeof categoryId._id === 'string' ? categoryId._id : String(categoryId._id);
            } else {
      ...
      if (!parentId || parentId === 'undefined' || parentId === 'null' || parentId === '[object Object]' || parentId.trim() === '') {
        setSubcategories([]);
        return;
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.standard_quantity && formData.standard_quantity !== '' && !isNaN(parseFloat(formData.standard_quantity))) {
        ...
        submitData.standard_quantity_unit = formData.standard_quantity_unit || 'kg';
      }
      ...
      if (product) {
        ...
        response = await adminServices.productCatalog.updateProduct(product._id, submitData);
      } else {
        // Create new product
        response = await adminServices.productCatalog.createProduct(submitData);
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-colors ${
                      validationErrors.product_type ? 'border-red-500 bg-red-50 dark:bg-red-900/10' : 'border-gray-300'
                  ...
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (validationErrors.color_code) {
                          setValidationErrors(prev => {
                            const newErrors = { ...prev };
                    ...
                    <input
                      ...
                      className={`flex-1 px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-colors ${
                        validationErrors.color_code ? 'border-red-500 bg-red-50 dark:bg-red-900/10' : 'border-gray-300'
                      }`}
                    />
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 flex items-center">
                    ...
                    <select
                    name="category_id"
                    value={normalizeId(formData.category_id)}
                    onChange={(e) => {
                      handleInputChange(e);
                    }}
                    className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-colors ${
                ...
                {formData.product_type === 'tinters' && groups.length > 0 && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 flex items-center">
                      <Package className="w-4 h-4 mr-2 text-gray-500" />
                      Group
                  ...
                  {formData.category_id && subcategories.length > 0 && (
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={() => {
                          const allIds = subcategories.map(sub => {
                            const id = sub._id;
                    ...
                    validationErrors.subcategory_ids 
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-[200px] overflow-y-auto">
                        {subcategories.map(subcategory => {
                          ...
                          const subcategoryIdStr = typeof subcategory._id === 'string' 
                            ? subcategory._id 
                            : (subcategory._id?.toString ? subcategory._id.toString() : String(subcategory._id));
                          const currentIdsStr = (formData.subcategory_ids || []).map(id => 
                            typeof id === 'string' ? id : (id?.toString ? id.toString() : String(id))
                          );
                          const isChecked = currentIdsStr.includes(subcategoryIdStr);
                          return (
                            <label
                              key={subcategoryIdStr}
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2 pb-2 border-b border-gray-200 dark:border-gray-700">
                ...
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Pricing & Units</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 flex items-center">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <select
                      ...
                      value={formData.standard_quantity_unit}
                      onChange={handleInputChange}
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2 pb-2 border-b border-gray-200 dark:border-gray-700">
                ...
                <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Chemical Properties</h3>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 flex items-center">
