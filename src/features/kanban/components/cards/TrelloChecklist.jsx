...
import { CheckSquare, Check, Plus, Trash2, MoreHorizontal, X } from 'lucide-react';
...
const TrelloChecklist = ({ checklist, onUpdate, onDelete }) => {
  ...
  const handleSaveEdit = () => {
    if (editText.trim()) {
      const updatedItems = items.map(item =>
        item.id === editingItemId ? { ...item, name: editText.trim() } : item
      );
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {completedCount > 0 && (
            ...
            >
              {hideCompleted ? 'Show' : 'Hide'} completed ({completedCount})
            </button>
          )}
