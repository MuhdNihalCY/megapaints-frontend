...
const Additives = () => {
    ...
    const fetchAdditives = async () => {
      try {
        ...
        if (response.success) {
          ...
          throw new Error(response.error || 'Failed to fetch additives');
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingAdditive) {
        ...
        response = await apiRequest(`/v1/additive/${editingAdditive._id}`, {
          method: 'PUT',
        ...
        if (response._id) {
          ...
          const updatedAdditives = additives.map(add => 
            add._id === editingAdditive._id ? { ...add, ...formData } : add
          );
          setAdditives(updatedAdditives);
          setSnackbar({ open: true, message: 'Additive updated successfully', severity: 'success' });
        } else {
          throw new Error(response.error || 'Failed to update additive');
        }
      } else {
        // Add new additive
        ...
        if (response._id) {
          ...
          setSnackbar({ open: true, message: 'Additive added successfully', severity: 'success' });
        } else {
      ...
      setSnackbar({ open: true, message: err.message || 'Failed to save additive', severity: 'error' });
  ...
  const handleDeleteAdditive = async (id) => {
    const confirmDelete = window.confirm('Are you sure you want to delete this additive?');
    ...
    try {
      ...
      if (response.message) {
        ...
        setSnackbar({ open: true, message: 'Additive deleted successfully', severity: 'success' });
      } else {
        throw new Error(response.error || 'Failed to delete additive');
      }
    } catch (err) {
      setSnackbar({ open: true, message: err.message || 'Failed to delete additive', severity: 'error' });
          ...
          <TableBody>
            {additives.map((additive) => (
              <TableRow key={additive._id}>
                ...
                <TableCell>{additive.price} {additive.priceUnit}</TableCell>
                <TableCell>
                  <IconButton
                  ...
                  <IconButton
                    color="error"
                    onClick={() => handleDeleteAdditive(additive._id)}
                  >
                    <DeleteIcon />
                  </IconButton>
                </TableCell>
