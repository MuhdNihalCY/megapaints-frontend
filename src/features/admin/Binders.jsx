...
const Binders = () => {
    ...
    const handleDeleteBinder = async (binder) => {
        if (
            !window.confirm(
                `Are you sure you want to delete binder "${binder.name}"? This action cannot be undone.`,
            )
        ) {
            return;
        }
    ...
    const filteredAndSortedBinders = useMemo(() => {
        ...
        if (searchTerm) {
            ...
            filtered = filtered.filter(
                (binder) =>
                    (binder.name &&
                        binder.name.toLowerCase().includes(searchLower)) ||
                    (binder.code &&
                        binder.code.toLowerCase().includes(searchLower)) ||
                    (binder.abbreviation &&
                        binder.abbreviation.toLowerCase().includes(searchLower)),
            );
        }
        if (filterActive !== null) {
            filtered = filtered.filter(
                (binder) => binder.is_active === filterActive,
            );
        }
        filtered.sort((a, b) => {
            let aValue, bValue;
                                        ...
                                        filteredAndSortedBinders.map(
                                            (binder) => (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                                                        {binder.density != null && binder.density !== ""
                                                            ? binder.density
                                                            : "-"}
                                                    </td>
                                                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
