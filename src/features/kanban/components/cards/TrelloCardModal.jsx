...
const TrelloCardModal = ({
    ...
    const [formData, setFormData] = useState(() => {
        if (card) {
            ...
            const reservationId =
                card.reservationId || card.reservation_id || reservation?.id;
            return {
                ...card,
    ...
    const transformCardToBackendFormat = (cardData) => {
        ...
        if (cardData.description !== undefined)
            transformed.description = cardData.description;
        if (cardData.priority !== undefined)
            transformed.priority = cardData.priority;
        ...
        if (cardData.startDate !== undefined) {
            transformed.start_date =
                cardData.startDate?.date || cardData.startDate || null;
        }
        ...
        if (cardData.column_id !== undefined)
            transformed.column_id = cardData.column_id;
        ...
        if (cardData.members !== undefined) {
            transformed.assignees = (cardData.members || []).map(
                (memberId) => ({
                    user_id: memberId,
                }),
            );
        ...
        if (cardData.labels !== undefined) {
            transformed.labels = (cardData.labels || [])
                .map((labelId) => {
                    ...
                    if (
                        ...
                        return {
                            label_id:
                                labelId.label_id || labelId.id || labelId._id,
                            name: labelId.name,
                            color: labelId.color,
                        };
                    }
                    ...
                    if (
                        ...
                        if (label) {
                            return {
                                ...
                                color:
                                    label.color || labelId.color || "#6b7280",
                            };
                        }
        ...
        // Ready products
        if (
            cardData.readyProducts !== undefined ||
            cardData.ready_products !== undefined
        ) {
            transformed.ready_products =
        ...
        try {
            ...
            if (onUpdate && typeof onUpdate === "function") {
                ...
                console.error("🔴 onUpdate is not a function", {
                    onUpdate,
                    type: typeof onUpdate,
                });
                throw new Error(
                    "Save handler not available. Please refresh the page.",
                );
            }
        } catch (error) {
    ...
    const handleCardUpdate = async (updates, options = {}) => {
        ...
        if (!cardId) {
            ...
            setError(
                "Cannot update: Card ID is missing. Please save the card first.",
            );
            return;
        }
        ...
        try {
            ...
            const backendUpdates =
                transformCardToBackendFormat(updatesWithLabels);
            ...
            if (options.logActivity && currentUser) {
                try {
                    const activity = {
                        ...
                        description:
                            options.activityDescription || "Card updated",
    ...
    useEffect(() => {
        ...
        if (card) {
            ...
            const identifier =
                ...
                reservation?.identifier ||
                (card.title
                    ? getCardTitleComponents(card.title).identifier
                    : null);
            ...
            const extractedLabels =
                card.labels ||
                (card.labelObjects || []).map(
                    (label) => label.id || label._id || label.label_id,
                ) ||
                [];
            ...
            const transformedAttachments = (card.attachments || []).map(
                (att) => {
                    ...
                    if (att.uploaded_at) {
                        ...
                        dateAdded =
                            att.dateAdded instanceof Date
                                ? att.dateAdded.toISOString()
                                : att.dateAdded;
                    } else {
            ...
            const customerNameFromTitle =
                ...
                !titleComponents.customerName
                    .toLowerCase()
                    .includes("restricted") &&
                titleComponents.customerName.toLowerCase() !==
                    "restricted access"
                    ? titleComponents.customerName
                    : null;
            ...
            if (card.customer) {
                ...
                if (
                    typeof card.customer === "object" &&
                    card.customer !== null &&
                    (card.customer._id || card.customer.id) &&
                    card.customer.name
                ) {
            ...
            if (titleToParse) {
                ...
                if (
                    ...
                    (components.customerSlug.toLowerCase() ===
                        "restricted-access" ||
                        components.customerSlug.toLowerCase() ===
                            "restrictedaccess" ||
                        components.customerSlug
    ...
    const handleCustomerSelect = async (customer) => {
        try {
            ...
            const completeTitle = generateCardTitle(
                titleComponents.identifier,
                customer.name,
            );
    ...
    const handleLabelToggle = async (labelId) => {
        try {
            ...
            const isLabelApplied = currentLabels.some(
                (l) => normalizeLabelId(l) === normalizedLabelId,
            );
            const label = labels.find((l) => {
                const lId = l.id || l._id;
            ...
            const formattedLabels = newLabels
                .map((labelIdOrObj) => {
                    ...
                    if (
                        typeof labelIdOrObj === "object" &&
                        labelIdOrObj.label_id &&
                        labelIdOrObj.name &&
                        labelIdOrObj.color
                    ) {
                        return {
        ...
        try {
            ...
            await handleCardUpdate(
                { closed: newClosedState },
                {
                    ...
                    activityDescription: newClosedState
                        ? "archived this card"
                        : "unarchived this card",
                    activityMetadata: {},
                },
            );
                                            ...
                                            <input
                                                ...
                                                onChange={async (e) => {
                                                    try {
                                                        ...
                                                        console.error(
                                                            "Failed to update due date completion:",
                                                            error,
                                                        );
                                                    }
                                                }}
                                    ...
                                    onMakeCover={async (attachment) => {
                                        try {
                                            ...
                                            if (
                                                ...
                                                throw new Error(
                                                    "Invalid attachment ID. The attachment may not be fully uploaded yet. Please wait a moment and try again.",
                                                );
                                            }
                                ...
                                <ReadyProductsManager
                                    ...
                                    onUpdate={async (readyProducts) => {
                                        try {
                                            ...
                                            console.error(
                                                "Failed to update ready products:",
                                                error,
                                            );
                                            setError(
                                                "Failed to update ready products: " +
                                    ...
                                    onUpdate={async (productionItems) => {
                                        try {
                                            ...
                                            setFormData((prev) => ({
                                                ...
                                                productionItems,
                                                production_items:
                                                    productionItems, // Also set backend format
                                            }));
                                            ...
                                            console.error(
                                                "Failed to update production items:",
                                                error,
                                            );
                                            setError(
                                                "Failed to update production items: " +
                                                            ...
                                                            if (editingLabel) {
                                                                await updateLabel(
                                                                        ...
                                                                        editingLabel._id,
                                                                    {
                                                                        ...
                                                                        description:
                                                                            labelFormData.description.trim(),
                                                                    },
                                                                );
                                                                toast.success(
