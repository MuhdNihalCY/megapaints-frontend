...
class LabelManagementService {
    ...
    calculateLabelStatistics(labels) {
        ...
        const mostUsedLabel = labels.reduce(
            (max, label) =>
    ...
    async getLabelUsageAnalytics(options = {}) {
        try {
            const labels = await this.getBranchLabels();
            const analytics = this.calculateUsageAnalytics(
                labels.data,
                options,
            );
    ...
    async bulkUpdateLabels(labelUpdates) {
        ...
        for (const update of labelUpdates) {
            try {
                const result = await this.updateBranchLabel(
                    update.labelId,
                    update.data,
                );
                results.successful.push(result.data);
    ...
    transformLabelToApi(labelData) {
        return {
            ...
            isActive:
                labelData.isActive !== undefined ? labelData.isActive : true,
        };
    }
    ...
    transformLabelFromApi(apiLabel) {
        return {
            ...
            isActive:
                apiLabel.isActive !== undefined ? apiLabel.isActive : true,
