...
const GroupForm = ({ group = null, onClose, onSuccess }) => {
    ...
    const validateForm = () => {
        ...
        if (
            ...
            !Array.isArray(formData.product_types) ||
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (group) {
                ...
                response = await adminServices.productCatalog.updateGroup(
                    group._id,
                    submitData,
                );
            } else {
                // Create new group
                response =
                    await adminServices.productCatalog.createGroup(submitData);
            }
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    >
                                        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                                            {PRODUCT_TYPES.map((type) => {
                                                const isChecked =
                                                    formData.product_types &&
                                                    formData.product_types.includes(
                                                        type.value,
                                                    );
                                                return (
                                                    <label
                                    ...
                                    <p className="mt-1.5 text-xs text-gray-500 dark:text-gray-400">
                                        Select one or more product types. This
                                        group will be available for the selected
                                        product types.
                                    </p>
                                    {validationErrors.product_types && (
                                        <p className="mt-1.5 text-sm text-red-600 dark:text-red-400 flex items-center">
