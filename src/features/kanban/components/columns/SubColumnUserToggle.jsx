...
const SubColumnUserToggle = ({ 
  ...
  const canToggle = currentUserDesignation === 'Office' || 
  ...
  const handleToggle = async (event) => {
    ...
    try {
      await kanbanService.toggleSubColumnUser(boardId, userId, columnId, newEnabled);
      
      if (onToggle) {
        onToggle(userId, columnId, newEnabled);
      }
      
      toast.success(newEnabled ? 'User enabled in sub-column' : 'User disabled in sub-column');
    } catch (error) {
      // Revert on error
      setLocalEnabled(!newEnabled);
  ...
  return (
    <Tooltip title={localEnabled ? 'Disable user in sub-column' : 'Enable user in sub-column'}>
      <span>
        <Switch
