...
const CategoryForm = ({
    ...
    useEffect(() => {
        if (category) {
            setFormData({
                ...
                binder_1_id:
                    category.binder_1_id?._id || category.binder_1_id || "",
                binder_2_id:
    ...
    const handleInputChange = (e) => {
        ...
        if (type === "checkbox") {
            ...
            if (
                name.startsWith("binder_1_equation_values.") ||
                name.startsWith("binder_2_equation_values.")
            ) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
                ...
                if (formData.binder_1_equation_values) {
                    ...
                    if (
                        ...
                        submitData.binder_1_equation_values = {
                            valueA:
                                eq1.valueA !== ""
                                    ? parseFloat(eq1.valueA)
                                    : null,
                            valueB:
                                eq1.valueB !== ""
                                    ? parseFloat(eq1.valueB)
                                    : null,
                            valueC:
                                eq1.valueC !== ""
                                    ? parseFloat(eq1.valueC)
                                    : null,
                            valueD:
                                eq1.valueD !== ""
                                    ? parseFloat(eq1.valueD)
                                    : null,
                        };
                    }
                }
                if (
                    formData.binder_2_equation_type !== undefined &&
                    formData.binder_2_equation_type !== ""
                ) {
                    submitData.binder_2_equation_type =
                        formData.binder_2_equation_type;
                }
            ...
            if (category) {
                ...
                response = await adminServices.productCatalog.updateCategory(
                    category._id,
                    submitData,
                );
            } else {
                // Create new category
                response =
                    await adminServices.productCatalog.createCategory(
                        submitData,
                    );
            }
    ...
    const getParentCategoryName = () => {
        ...
        if (formData.parent_id) {
            const parent = categories.find(
                (cat) => cat._id === formData.parent_id,
            );
                                    ...
                                    <div className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600">
                                        ...
                                        <div className="mb-3 p-2 bg-white dark:bg-gray-800 rounded border border-gray-200 dark:border-gray-600">
                                            <code className="text-xs text-gray-600 dark:text-gray-400">
                                                (( Total tinter X valueA X{" "}
                                                {formData.level_of_shine ||
                                                    "Gloss or Matt"}{" "}
                                                ) - (valueB X valueC X Total
                                                tinter ) ) /valueD
                                            </code>
                                        </div>
                                        <div className="grid grid-cols-2 gap-3">
