...
export async function fetchMastersWithCache() {
    ...
    let etag = null;
    try {
        etag = localStorage.getItem(ETAG_KEY) || null;
    } catch (_) {}

    try {
        const res = await Promise.all([
            api.get("/v1/categories", { params: { page: 1, limit: 1000 } }),
            api.get("/v1/subcategories", { params: { page: 1, limit: 1000 } }),
            api.get("/v1/products", { params: { page: 1, limit: 1000 } }),
            api.get("/v1/products/additives", { params: { page: 1, limit: 1000 } }),
        ]);
        ...
        const rawCategories =
            (Array.isArray(res[0]?.data?.categories) &&
        ...
        const rawSubcategories =
            (Array.isArray(res[1]?.data?.subcategories) &&
                res[1].data.subcategories) ||
            (Array.isArray(res[1]?.data?.Subcategories) &&
                res[1].data.Subcategories) ||
        ...
        rawCategories.forEach((cat) => {
            const name =
                ...
                cat?._id;
            const id =
                cat?.Category_Id || cat?._id || cat?.id || cat?.CategoryID;
        ...
        rawSubcategories.forEach((sub) => {
            ...
            if (categoryId && categoryIdToName.has(String(categoryId))) {
                const parentCategoryName = categoryIdToName.get(
                    String(categoryId),
                );
        ...
        const rawProducts = Array.isArray(res[2]?.data?.products)
            ? res[2].data.products
            : [];
        if (
        ...
        try {
            localStorage.setItem(ETAG_KEY, syntheticEtag);
        } catch (_) {}
        try {
...
export async function fetchMastersFresh() {
    try {
        ...
        const res = await Promise.all([
            ...
            api.get("/v1/products", { params: { page: 1, limit: 1000 } }),
            api.get("/v1/products/additives", { params: { page: 1, limit: 1000 } }),
            api.get("/v1/products/binders", { params: { page: 1, limit: 1000 } }),
        ]);

        // Extract raw data with fallback handling
        const rawCategories = extractArrayData(res[0]?.data, [
            "categories",
            "category",
            "data",
            "items",
        ]);
        const rawSubcategories = extractArrayData(res[1]?.data, [
            "subcategories",
            "Subcategories",
            "data",
            "items",
        ]);
        const rawProducts = extractArrayData(res[2]?.data, [
            "products",
            "data",
            "items",
        ]);
        const rawAdditives = extractArrayData(res[3]?.data, [
            "additives",
            "data",
            "items",
        ]);
        const rawBinders = extractArrayData(res[4]?.data, [
            "binders",
            "data",
            "items",
        ]);
        ...
        if (process.env.NODE_ENV === 'development') {
            console.log('[Masters API] Data extraction summary:', {
                ...
                binderApiResponse: res[4]?.data ? {
                    hasBindersKey: 'binders' in res[4].data,
                    hasDataKey: 'data' in res[4].data,
                    hasItemsKey: 'items' in res[4].data,
                    topLevelKeys: Object.keys(res[4].data),
                    status: res[4].data.status
                } : 'No response'
            });
        }
        ...
        rawCategories.forEach((cat) => {
            const id =
                cat?.Category_Id || cat?._id || cat?.id || cat?.CategoryID;
            const name =
                cat?.Category || cat?.name || cat?.Category_Name || cat?.label;
        ...
        rawBinders.forEach((binder) => {
            const binderId = String(
                binder?.Binder_Id || binder?._id || "",
            ).trim();
            ...
            if (
                ...
                binderMap.set(binderId, {
                    ...
                    Abbreviation:
                        binder?.Abbreviation || binder?.abbreviation || "",
                    Description:
        ...
        rawSubcategories.forEach((sub) => {
            const subId = String(
                sub?.SubCategory_Id || sub?._id || sub?.id || "",
            );
            const subName =
                sub?.SubCategory ||
                sub?.name ||
            ...
            if (parentCategoryId) {
                ...
                const existing = subCategoriesByCategory[parentCategoryId].find(
                    (s) => s.id === subId,
                );
                if (!existing) {
                ...
                // The API returns binder_1_id and binder_2_id (with underscores)
                const binder1Id = String(
                    sub?.binder_1_id?._id || // Populated object
                    sub?.binder_1_id || // Direct ID
                    sub?.Binder1 || // Legacy field name
                    sub?.Products?.Binder1 || // Nested legacy
                    "",
                );
                const binder2Id = String(
                    sub?.binder_2_id?._id || // Populated object
                    sub?.binder_2_id || // Direct ID
                    sub?.Binder2 || // Legacy field name
                    sub?.Products?.Binder2 || // Nested legacy
                    "",
                );

                // Debug log in development to track binder ID extraction
                if (process.env.NODE_ENV === 'development' && (binder1Id || binder2Id)) {
                    console.log(`[Masters Service] Subcategory "${subName}" binders:`, {
                        binder1Id,
                        binder2Id,
                        source: sub?.binder_1_id ? 'binder_1_id field' : 
                                sub?.Binder1 ? 'Binder1 field' : 'not found'
                    });
                }
                ...
                binderConfigBySubCategory[subId] = {
                    ...
                    Binder1Name:
                        binder1Data?.Binder_Name || `Binder ${binder1Id}`,
                    Binder2Name:
                        binder2Data?.Binder_Name || `Binder ${binder2Id}`,

                    // Binder configuration values
                    // Handle both new field names (binder_1_equation_values) and legacy (Binder1Avalue)
                    Binder1Avalue: Number(
                        sub?.binder_1_equation_values?.valueA ||
                        sub?.Binder1Avalue ||
                        0
                    ),
                    Binder1Bvalue: Number(
                        sub?.binder_1_equation_values?.valueB ||
                        sub?.Binder1Bvalue ||
                        0
                    ),
                    Binder1Cvalue: Number(
                        sub?.binder_1_equation_values?.valueC ||
                        sub?.Binder1Cvalue ||
                        0
                    ),
                    Binder1dvalue: Number(
                        sub?.binder_1_equation_values?.valueD ||
                        sub?.Binder1dvalue ||
                        1
                    ),
                    Binder2Avalue: Number(
                        sub?.binder_2_equation_values?.valueA ||
                        sub?.Binder2Avalue ||
                        0
                    ),
                    Binder2EQ1: 
                        sub?.binder_2_equation_type === "eq1" ||
                        Boolean(sub?.Binder2EQ1),
                    ...
                    // UI configuration
                    // Handle both new field names (level_of_shine) and legacy (Gloss/Matt)
                    Gloss: 
                        sub?.level_of_shine === "gloss" ||
                        Boolean(sub?.Gloss || sub?.Products?.Gloss),
                    Matt: 
                        sub?.level_of_shine === "matt" ||
                        Boolean(sub?.Matt || sub?.Products?.Matt),
                    Liter: 
                        sub?.unit === "liter" ||
                        Boolean(sub?.Liter || sub?.Products?.Liter),
                    Gram: 
                        sub?.unit === "gram" ||
                        Boolean(sub?.Gram || sub?.Products?.Gram),
                    ...
                    brand: sub?.brand || "",
                    unit: sub?.unit || "",
                    level_of_shine: sub?.level_of_shine || "",
                };
        ...
        const defaultSubCategory =
            subCategoriesByCategory[defaultCategory]?.[0]?.id || "";
