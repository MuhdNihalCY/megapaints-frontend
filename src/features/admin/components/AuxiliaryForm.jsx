...
const AuxiliaryForm = ({ auxiliary = null, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (auxiliary) {
      setFormData({
        ...
        type: auxiliary.type || '',
        properties: typeof auxiliary.properties === 'object' ? JSON.stringify(auxiliary.properties) : (auxiliary.properties || ''),
        base_price: auxiliary.unit_price || '',
        unit: auxiliary.unit || 'kg',
        is_active: auxiliary.is_active !== undefined ? auxiliary.is_active : true,
      });
    }
  }, [auxiliary]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (auxiliary) {
        ...
        response = await adminServices.productCatalog.updateAuxiliary(auxiliary._id, submitData);
      } else {
        // Create new auxiliary
        response = await adminServices.productCatalog.createAuxiliary(submitData);
      }

      if (response.status === 'success') {
        setSuccess(auxiliary ? 'Auxiliary updated successfully!' : 'Auxiliary created successfully!');
        setTimeout(() => {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">
                {auxiliary ? 'Edit Auxiliary' : 'Add New Auxiliary'}
              </h2>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {auxiliary ? 'Update auxiliary information' : 'Create a new paint auxiliary'}
              </p>
            </div>
          </div>
          <button
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2 pb-2 border-b border-gray-200 dark:border-gray-700">
                <PuzzlePiece className="w-5 h-5 text-green-600 dark:text-green-400" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    value={formData.code}
                    onChange={handleInputChange}
                    className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-colors uppercase ${
                ...
                <div className="md:col-span-2">
                  ...
                  <textarea
                    ...
                    onChange={handleInputChange}
                    rows={3}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-colors"
                    placeholder="Enter auxiliary description"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white transition-colors"
                    placeholder="Enter properties (JSON or text)"
                  />
                </div>
              </div>
            </div>
            ...
            <div>
              <label className="flex items-center">
                ...
                />
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Active Auxiliary</span>
              </label>
            </div>
            ...
            <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-6 py-2.5 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 disabled:from-gray-400 disabled:to-gray-500 text-white rounded-lg transition-all duration-200 flex items-center font-medium shadow-md hover:shadow-lg transform hover:scale-105 disabled:transform-none disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
                  ...
                  <>
                    ...
                    {auxiliary ? 'Update Auxiliary' : 'Create Auxiliary'}
                  </>
                )}
              </button>
            </div>
