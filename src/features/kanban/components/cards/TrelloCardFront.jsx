...
const TrelloCardFront = ({ 
  ...
  const cardLabels = (card.labels || [])
    .map(labelId => labels.find(l => l.id === labelId || l._id === labelId))
    .filter(Boolean);
  ...
  const getDueDateColor = () => {
    ...
    if (badges.dueDate.isComplete) return 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300';
    if (badges.dueDate.isOverdue) return 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300';
    if (badges.dueDate.isDueSoon) return 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-300';
      ...
      <div className={hasCover ? 'p-2' : ''}>
          ...
          <div className="flex flex-wrap gap-1 mb-2">
            {cardLabels.slice(0, 6).map((label) => (
              <div
                ...
                `}
                style={{
                  backgroundColor: label.color && label.color !== 'gray' ? label.color : 'transparent'
                }}
                title={label.name}
              >
                {(label.color === 'gray' || !label.color) && label.name}
              </div>
            ))}
            {cardLabels.length > 6 && (
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          {badges.checklist.total > 0 && (
            <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded ${getChecklistColor()}`}>
              <CheckSquare className="w-3 h-3" />
              <span>{badges.checklist.completed}/{badges.checklist.total}</span>
            </div>
          )}
          
          {/* Due Date */}
          {badges.dueDate && (
            <div className={`flex items-center gap-1 px-1.5 py-0.5 rounded ${getDueDateColor()}`}>
              <Clock className="w-3 h-3" />
              <span>{badges.dueDate.text}</span>
              {badges.dueDate.isComplete && <span className="ml-0.5">✓</span>}
            </div>
          )}
