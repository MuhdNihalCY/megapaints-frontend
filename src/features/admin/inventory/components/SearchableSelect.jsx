...
const SearchableSelect = ({
  ...
  useEffect(() => {
    if (searchTerm) {
      const filtered = options.filter(option => {
        ...
        const primaryText = String(option[searchKey] || '').toLowerCase();
        const secondaryText = secondaryKey ? String(option[secondaryKey] || '').toLowerCase() : '';
        return primaryText.includes(searchValue) || secondaryText.includes(searchValue);
      });
  ...
  const selectedOption = options.find(opt => opt._id === value || opt.id === value);
        ...
        <div className="flex items-center px-4 py-2">
          {selectedOption ? (
            <div className="flex-1 text-gray-900 dark:text-white">
              {selectedOption[displayKey] || selectedOption[searchKey]}
            ...
            {filteredOptions.length > 0 ? (
              filteredOptions.map((option, idx) => {
                ...
                if (optionValue) {
                  ...
                  } else if (optionValue.toString && typeof optionValue.toString === 'function') {
                    const str = optionValue.toString();
                    keyValue = str !== '[object Object]' ? str : `option-${idx}`;
                  } else {
                    keyValue = `option-${idx}`;
                  }
                } else {
                ...
                return (
                  ...
                  >
                    <div className="text-sm text-gray-900 dark:text-white">
                      {option[displayKey] || option[searchKey]}
                    </div>
