...
const Tinters = () => {
  ...
  const fetchTinters = async () => {
    try {
      ...
      const params = {
        ...
        product_type: 'tinters', // Filter by tinters product type
      ...
      if (response.status === 'success') {
        setTinters(response.data.products || []);
        setPagination(prev => ({
          ...prev,
        ...
        setError('Failed to fetch tinters');
      }
    } catch (err) {
      console.error('Failed to fetch tinters:', err);
      setError(err.message || 'Failed to fetch tinters');
    } finally {
      setLoading(false);
    }
  };

  const handleAddTinter = () => {
    setEditingTinter(null);
    setShowForm(true);
  };

  const handleEditTinter = (tinter) => {
    setEditingTinter(tinter);
    setShowForm(true);
  };

  const handleDeleteTinter = async (tinter) => {
    if (!window.confirm(`Are you sure you want to delete tinter "${tinter.name}"? This action cannot be undone.`)) {
      return;
    }

    try {
      const adminServices = getAdminServices();
      const response = await adminServices.productCatalog.deleteProduct(tinter._id);

      if (response.status === 'success') {
        setSuccess('Tinter deleted successfully');
        fetchTinters();
        setTimeout(() => setSuccess(''), 3000);
      } else {
        setError('Failed to delete tinter');
      }
    } catch (err) {
      console.error('Failed to delete tinter:', err);
      setError(err.message || 'Failed to delete tinter');
    }
  };
  ...
  const filteredAndSortedTinters = useMemo(() => {
    let filtered = [...tinters];
    
    // Apply search filter
    if (searchTerm) {
      const searchLower = searchTerm.toLowerCase();
      filtered = filtered.filter(tinter => 
        (tinter.name && tinter.name.toLowerCase().includes(searchLower)) ||
        (tinter.code && tinter.code.toLowerCase().includes(searchLower))
      );
    }
    
    // Apply active filter
    if (filterActive !== null) {
      filtered = filtered.filter(tinter => tinter.is_active === filterActive);
    }
    ...
    return filtered;
  }, [tinters, sortField, sortDirection, searchTerm, filterActive]);

  if (loading && tinters.length === 0) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center space-y-4">
          <Activity className="w-8 h-8 text-blue-500 animate-spin" />
          <p className="text-gray-600 dark:text-gray-400">Loading tinters...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Tinters Management</h2>
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Manage tinters products and groups
          </p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={handleAddTinter}
            ...
            <div className="overflow-hidden">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredAndSortedTinters.length === 0 ? (
                    <tr>
                    ...
                    <p className="text-gray-400 dark:text-gray-500 text-sm mt-2">
                      {searchTerm || filterActive !== null
                        ...
                        : 'Get started by adding a new tinter'}
                    </p>
                  </td>
                </tr>
              ) : (
                filteredAndSortedTinters.map((tinter) => (
                  <tr key={tinter._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                      ...
                      <div className="flex items-center">
                        <div className="min-w-0">
                          ...
                          <div className="text-xs sm:text-sm text-gray-500 dark:text-gray-400 truncate">
                            Code: {tinter.code || tinter._id?.toString().slice(-8) || tinter.id}
                          </div>
                          {tinter.description && (
                    ...
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white hidden lg:table-cell">
                      <div className="flex items-center">
                        <span className="font-semibold">AED {tinter.base_price?.toFixed(2) || '0.00'}</span>
                        <span className="text-xs text-gray-500 dark:text-gray-400 ml-1 whitespace-nowrap">
                          / {tinter.unit || 'unit'}
                        </span>
                      </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-700/50 px-3 sm:px-6 py-4 border-t border-gray-200 dark:border-gray-700">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="text-xs sm:text-sm text-gray-700 dark:text-gray-300">
                ...
                of <span className="font-medium">{pagination.total}</span> tinters
              </div>
          ...
          <div className="bg-gray-50 dark:bg-gray-700/50 px-3 sm:px-6 py-3 border-t border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div className="flex items-center text-sm text-gray-700 dark:text-gray-300">
                ...
                <span className="font-medium">{pagination.total}</span> total tinters
              </div>
            </div>
          </div>
        )}
