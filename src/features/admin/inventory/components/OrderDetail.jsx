import { X, Package, User, MapPin, CreditCard, Calendar, FileText, Store } from 'lucide-react';
...
const OrderDetail = ({ order, onClose, onStatusChange }) => {
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`px-3 py-1 text-sm font-semibold rounded-full ${statusColors[status] || statusColors.pending}`}>
          ...
          <div className="bg-white dark:bg-gray-800 px-6 py-4 max-h-[calc(100vh-200px)] overflow-y-auto">
                ...
                <div>
                  ...
                  <dl className="space-y-2">
                    <div>
                      ...
                      <dd className="text-sm font-medium text-gray-900 dark:text-white">{order.order_number}</dd>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <dd className="text-sm text-gray-900 dark:text-white">{formatDate(order.created_at)}</dd>
                    </div>
                    {order.updated_at && (
                      <div>
                        <dt className="text-sm text-gray-500 dark:text-gray-400">Last Updated</dt>
                        <dd className="text-sm text-gray-900 dark:text-white">{formatDate(order.updated_at)}</dd>
                      </div>
                    )}
                  </dl>
                </div>
                  ...
                  <dl className="space-y-2">
                    ...
                    {order.customer?.email && (
                      <div>
                        ...
                        <dd className="text-sm text-gray-900 dark:text-white">{order.customer.email}</dd>
                      </div>
                    )}
                    {order.customer?.phone && (
                      <div>
                        <dt className="text-sm text-gray-500 dark:text-gray-400">Phone</dt>
                        <dd className="text-sm text-gray-900 dark:text-white">{order.customer.phone}</dd>
                      </div>
                    )}
                    {order.customer?.customer_type && (
                      <div>
                        <dt className="text-sm text-gray-500 dark:text-gray-400">Customer Type</dt>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <dl className="space-y-2">
                    ...
                    {order.payment_info?.amount_paid > 0 && (
                      <div>
                        <dt className="text-sm text-gray-500 dark:text-gray-400">Amount Paid</dt>
                        <dd className="text-sm text-gray-900 dark:text-white">
                          {formatCurrency(order.payment_info.amount_paid)}
                        </dd>
                      </div>
                    )}
                  </dl>
            ...
            <div className="mt-6">
              <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-3 flex items-center">
                ...
                {order.order_type === 'formula' ? 'Formula Details' : 'Order Items'}
              </h4>
              ...
              {order.order_type === 'formula' ? (
                <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 space-y-4">
                  {order.formula_data?.file_number && (
                    <div>
                      <dt className="text-sm text-gray-500 dark:text-gray-400">File Number</dt>
                      <dd className="text-sm font-medium text-gray-900 dark:text-white">
                        {order.formula_data.file_number}
                      </dd>
                    </div>
                  ...
                  {order.formula_data?.subcategory && (
                    <div>
                      <dt className="text-sm text-gray-500 dark:text-gray-400">Subcategory</dt>
                      <dd className="text-sm text-gray-900 dark:text-white">
                        {order.formula_data.subcategory}
                      </dd>
                    </div>
                  ...
                  {order.quantities && (
                    <div>
                      ...
                      <dd className="text-sm text-gray-900 dark:text-white">
                        {order.quantities.requested} {order.quantities.unit}
                      </dd>
                    </div>
                  )}
                </div>
                  ...
                  {order.status_history.map((history, index) => (
                    <div key={index} className="flex items-start space-x-3 text-sm">
                      <div className="flex-1">
                        ...
                        {history.notes && (
                          <p className="text-gray-600 dark:text-gray-400 mt-1">{history.notes}</p>
                        )}
                      </div>
                    </div>
                  ))}
