  ...
  useEffect(() => {
    if (binder) {
      setFormData({
        ...
        unit: binder.unit || 'kg',
        is_active: binder.is_active !== undefined ? binder.is_active : true,
      });
    }
  }, [binder]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (binder) {
        ...
        response = await adminServices.productCatalog.updateBinder(binder._id, submitData);
      } else {
        // Create new binder
        response = await adminServices.productCatalog.createBinder(submitData);
      }

      if (response.status === 'success') {
        setSuccess(binder ? 'Binder updated successfully!' : 'Binder created successfully!');
        setTimeout(() => {
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {binder ? 'Update binder information' : 'Create a new paint binder'}
              </p>
            </div>
          </div>
          <button
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="flex items-center">
                ...
                />
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Active Binder</span>
              </label>
            </div>
            ...
            <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-6 py-2.5 bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 disabled:from-gray-400 disabled:to-gray-500 text-white rounded-lg transition-all duration-200 flex items-center font-medium shadow-md hover:shadow-lg transform hover:scale-105 disabled:transform-none disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
